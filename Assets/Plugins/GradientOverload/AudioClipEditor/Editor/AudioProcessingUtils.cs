...
namespace AudioClipEditor
{
    public static class AudioProcessingUtils
    {
        ...
        public static void ApplyModificationsFromSettings(AudioClip clip)
        {
            ...
            ApplyFade(modifiedSamples, modifiedSamples.Length, sampleRate, fadeInDuration, fadeOutDuration, fadeInCurve,
                fadeOutCurve);
            if (normalize) Normalize(modifiedSamples);
            AdjustVolume(modifiedSamples, volume);

            AudioClip modifiedClip = AudioClip.Create(clip.name, modifiedSamples.Length / channelCount, clip.channels,
                sampleRate, false);
        ...
        public static void ApplyFade(float[] samples, int length, int sampleRate, float fadeInDuration,
            float fadeOutDuration, AnimationCurve fadeInCurve, AnimationCurve fadeOutCurve)
        {
        ...
        public static string GetUneditedClipPath(AudioClip clip)
        {
            ...
            {
                Debug.LogError(
                    "No AudioClipEditorAnchor script found in the project! It is required to determine the folder for unedited clips.");
                return null;
            }
