...
namespace PampelGames.GoreSimulator
{
    public class GoreMultiCut : MonoBehaviour, IGoreObjectParent
    {
        ...
        private List<GameObject> ExecuteExplosionInternal(Vector3 position, float force, Vector3 centerPosition)
        {
            ...
            {
                var subModuleObjectClass = new SubModuleObjectClass();
                subModuleObjectClass.obj = child.childObject;
                subModuleObjectClass.boneTag = bonesClass.goreBone.boneTag;
                ...
                var worldCenter = subModuleObjectClass.obj.transform.TransformPoint(subModuleObjectClass.mesh.bounds.center);
                subModuleObjectClass.centerPosition = worldCenter;
                if (force != 0) subModuleObjectClass.force = (worldCenter - position).normalized * force;
        ...
        private void ExecuteCutInternal(string boneName, Vector3 position, Vector3 force, out GameObject detachedObject)
        {
            ...
            var children01 = new List<MultiCutChildClass>();
            var children02 = new List<MultiCutChildClass>();
            ...
            foreach (var multiCutChildClass in multiCutChildClassList)
            {
                ...
                var distance = (world - position).sqrMagnitude;
            ...
            if (children02.Count == 0)
            {
                ...
                {
                    for (var i = children01.Count - 1; i >= 0; i--)
                    {
                ...
                else // Going through children, detach the nearest one.
                {
                    ...
                    var closestChildDistance = float.MaxValue;
                    ...
                    for (var i = 0; i < children02.Count; i++) children01.Remove(children02[i]);
                }
            }

            /********************************************************************************************************************************/

            if (children01.Count < children02.Count) (children01, children02) = (children02, children01);
            ...
            if (children02.Count > 0)
            {
                ...
                subModuleClass.cutDirection = math.normalizesafe(worldBounds02 - worldBounds01);
            }
            ...
            if (children02.Count > 0)
            {
                ...
                {
                    if (children02[i].childObject.TryGetComponent<GoreMesh>(out var goreMesh))
                        goreMesh._goreMultiCut = goreMultiCut;
                    var subModuleObjectClass = new SubModuleObjectClass
                    {
                        obj = children02[i].childObject,
                        renderer = children02[i].childObject.GetComponent<Renderer>(),
                        mesh = children02[i].chunkClass.mesh
                    };
                ...
                var destroyableObjects = new List<GameObject>();
                destroyableObjects.Add(detachedObject);
                var poolableObjects = new List<GameObject>();
                for (var k = 0; k < _goreSimulator.cutModules.Count; k++)
                {
                    if (!_goreSimulator.cutModules[k].moduleActive) continue;
            ...
            for (var k = 0; k < _goreSimulator.cutModules.Count; k++) _goreSimulator.cutModules[k].ExecuteModuleCut(subModuleClass);
        ...
        private void CreateChildObjects()
        {
            ...
            var currentChildren = new List<GameObject>(transform1.childCount);
            foreach (Transform child in transform) currentChildren.Add(child.gameObject);

            foreach (var kvp in childrenDict)
            foreach (var multiCutChildClass in kvp.Value)
            {
                ...
                if (detachedChild.TryGetComponent<Renderer>(out var _renderer)) _renderer.materials = _goreSimulator.GetInstancedMaterialsStatic();
        ...
        private void AddChunkChildren(List<MultiCutChildClass> multiCutChildClasses, string boneName,
            Dictionary<string, List<MultiCutChildClass>> dictionary)
        {
            if (!_goreSimulator.bonesDict.TryGetValue(boneName, out var tuple)) return;
            for (var i = 0; i < tuple.Item1.boneChildrenSel.Count; i++)
            {
        ...
        internal void CreateMultiCutChildClassDict(List<MultiCutChildClass> multiCutChildClasses)
        {
            ...
            for (var i = 0; i < multiCutChildClasses.Count; i++)
            {
