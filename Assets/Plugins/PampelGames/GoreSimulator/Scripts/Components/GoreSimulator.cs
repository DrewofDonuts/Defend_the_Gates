    ...
    public class GoreSimulator : MonoBehaviour, IGoreObjectParent
    {
        ...
        private void InitializeMeshCut()
        {
            ...
            if(Application.isPlaying)
            {
                ...
                instancedMaterials = smr.materials.Select(Instantiate).ToArray();
                instancedMaterialsStatic = smr.materials.Select(Instantiate).Concat(new[] {Instantiate(cutMaterialStatic), Instantiate(cutMaterialStatic)}).ToArray();
                Pool.InitializePool(_defaultReferences.pooledMesh, _globalSettings.cutPreload, false);
            }
            else
            {
        ...
        internal void AddCutMaterial()
        {
            ...
            var materials = Application.isPlaying ? smr.materials : smr.sharedMaterials;
            Array.Resize(ref materials, materials.Length + 1);
            materials[^1] = cutMaterial;
            if (Application.isPlaying) smr.materials = materials;
            else smr.sharedMaterials = materials;
        ...
        internal Material[] GetInstancedMaterials()
        {
        ...
        public void DespawnDetachedObjects()
        {
            ...
            if (_globalSettings.poolActive && Application.isPlaying)
