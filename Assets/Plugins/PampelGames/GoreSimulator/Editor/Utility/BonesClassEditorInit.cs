...
namespace PampelGames.GoreSimulator.Editor
{
    internal static class BonesClassEditorInit
    {
        ...
        public static bool FillBonesClasses(GoreSimulator _goreSimulator, Mesh bakedMesh)
        {
            ...
            for (var i = 0; i < orderedBones.Count; i++)
            {
                ...
                var boneIndexesToAdd = MeshEditorUtility.GetBoneIndexes(_goreSimulator.smr, bonesClass.bone, _goreSimulator.weightsThreshold);
            ...
            for (var i = 0; i < _goreSimulator.bonesClasses.Count; i++)
            {
                ...
                void TraverseChildren(Transform bone, SkinnedMeshRenderer smr)
                {
                    foreach (Transform childBone in bone)
                    {
                        ...
                        var boneIndexesToAdd = MeshEditorUtility.GetBoneIndexes(_goreSimulator.smr, childBone, _goreSimulator.weightsThreshold);
            ...
            for (var i = 0; i < _goreSimulator.bonesClasses.Count; i++)
            {
                ...
                void TraverseParents(Transform bone, SkinnedMeshRenderer smr)
                {
                    ...
                    {
                        var boneIndexesToAdd = MeshEditorUtility.GetBoneIndexes(_goreSimulator.smr, parentBone, _goreSimulator.weightsThreshold);
                        bonesClass.boneIndexes.AddRange(boneIndexesToAdd.Except(bonesClass.boneIndexes));
                    }
            ...
            for (var i = 0; i < bonesClasses.Count; i++)
            {
                ...
                for (var j = 0; j < bonesStorageClass.cutIndexClasses.Count; j++)
                {
                    ...
                    MeshCutJobs.IndexesSnapshotCut(meshDataClass, meshNativeDataClass, bakedMesh, executionCutClass, 
                        explosionMesh, true);
