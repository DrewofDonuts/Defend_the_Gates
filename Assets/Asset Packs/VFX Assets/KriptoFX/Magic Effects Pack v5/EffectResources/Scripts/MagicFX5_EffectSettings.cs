...
# endif
    ...
    void TriggerAnimator(Transform target)
    {
        ...
        if (currentAnimator == null)
            currentAnimator =
                target.GetComponentInParent<Animator>(); //try again to find an animator, for example when collided body parts
    ...
    internal Vector3 GetTargetCenter(Transform target)
    {
        ...
        if (cache.SkinnedMeshRenderer != null)
            return cache.SkinnedMeshRenderer.bounds.center + Vector3.up * TargetCenterHeightOffset;
