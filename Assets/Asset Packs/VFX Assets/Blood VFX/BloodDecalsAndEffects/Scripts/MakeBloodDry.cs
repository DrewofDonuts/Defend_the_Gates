...
public class MakeBloodDry : MonoBehaviour
{
    ...
    {
        rend = GetComponent<Renderer>();
        if (rend.material.HasProperty("_DriedOpacityLevel"))
        {
            fadeAmount = rend.material.GetFloat("_DriedOpacityLevel");
        }
        else
        {
            Debug.LogWarning("Material does not have a '_DriedOpacityLevel' property.");
        }
    }
    ...
    void Update()
    {
        ...
        {
            bloodDryAmount = 1.0f;
        } // limit blood dry level

        if (bloodDryAmount < 0.0f)
        {
            bloodDryAmount = 0.0f;
        } // limit blood dry level
